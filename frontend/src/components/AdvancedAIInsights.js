    ...
    <div style={{ display: "grid", gap: 12 }}>
      {successRates?.overall?.decisions?.total > 0 && (
        <section style={{ ...aiCard(palette), padding: 14 }}>
          <h3 style={{ ...h3, color: palette.text }}><ChartBarIcon style={{ ...icon, color: palette.muted }} /> Success Rates</h3>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(132px,1fr))", gap: 10 }}>
            <Metric value={`${successRates.overall.decisions.rate}%`} label="Overall" palette={palette} />
            {Object.entries(successRates.by_impact || {}).map(([impact, data]) =>
              data.total > 0 ? <Metric key={impact} value={`${Math.round(data.rate)}%`} label={`${impact} impact`} palette={palette} /> : null
            )}
          </div>
        </section>
      )}
      ...
      {bottlenecks.length > 0 && (
        <section style={{ ...aiCard(palette), padding: 14 }}>
          <h3 style={{ ...h3, color: palette.text }}><ExclamationTriangleIcon style={{ ...icon, color: palette.danger }} /> Bottlenecks</h3>
          <div style={{ display: "grid", gap: 10 }}>{bottlenecks.map((b, i) => <Notice key={i} text={b.message} level={b.severity} palette={palette} />)}</div>
        </section>
      )}

      {gaps.length > 0 && (
        <section style={{ ...aiCard(palette), padding: 14 }}>
          <h3 style={{ ...h3, color: palette.text }}><DocumentMagnifyingGlassIcon style={{ ...icon, color: palette.warm }} /> Knowledge Gaps</h3>
          <div style={{ display: "grid", gap: 10 }}>{gaps.map((g, i) => <Notice key={i} text={g.message} level={g.severity} palette={palette} />)}</div>
        </section>
      )}

      {patterns.length > 0 && (
        <section style={{ ...aiCard(palette), padding: 14 }}>
          <h3 style={{ ...h3, color: palette.text }}><LightBulbIcon style={{ ...icon, color: palette.accent }} /> Patterns</h3>
          <div style={{ display: "grid", gap: 10 }}>
            {patterns.map((p, i) => (
              <div key={i} style={{ ...box, border: `1px solid ${palette.border}`, background: palette.cardAlt }}>
                <p style={{ ...pText, color: palette.text }}>{p.message}</p>
              </div>
            ))}
          </div>
        </section>
...
function Metric({ value, label, palette }) {
  return (
    <div style={{ ...box, border: `1px solid ${palette.border}`, background: palette.cardAlt, padding: 10 }}>
      <p style={{ margin: 0, fontSize: 20, fontWeight: 800, color: palette.text }}>{value}</p>
      <p style={{ margin: "4px 0 0", fontSize: 11, color: palette.muted, textTransform: "capitalize" }}>{label}</p>
    </div>
  );
}
