...
def projects(request):
    ...
    if request.method == 'GET':
        ...
        try:
            ...
            if not key or not request.data.get('name'):
                return Response({'error': 'name and key required'}, status=400)
...
def issues(request, project_id):
    ...
    if request.method == 'GET':
        issues = project.issues.select_related('assignee', 'reporter', 'sprint').all()
    ...
    try:
        ...
        return Response({
            ...
            'issue_stats': {
                ...
                'in_progress': issues.filter(status='in_progress').count(),
                'todo': issues.filter(status='todo').count(),
                'completion_percentage': int((completed / issues.count() * 100) if issues.count() > 0 else 0)
            },
            'blockers': [{
                'id': b.id,
                'title': b.title,
...
def blockers(request):
    ...
    if request.method == 'GET':
        ...
        try:
            sprint_id = request.data.get('sprint_id')
            if not sprint_id or not request.data.get('title'):
                return Response({'error': 'sprint_id and title required'}, status=400)
...
def project_issues_unified(request, project_id):
    ...
    try:
        issues = Issue.objects.filter(
            project_id=project_id,
            organization=org
        ).select_related('assignee', 'sprint').prefetch_related('labels')
